...
const guideSections: GuideSection[] = [
  {
    ...
    content: (
      <>
        {/* 音声再生に関する注意書きを修正（カタカナ表記を追加） */}
        <p style={{ color: '#ff6b6b', fontWeight: 'bold' }}>【重要】現在、一部の漢字で音声再生が正しく行われない問題を確認しています。対象: 揈(fing フェン), 搲(we ウェー), 啤(pe ペー), 掉(deu デーウ), 𠺫(leu レーウ), 嗒(dep デッ(プ)), 夾(gep ゲッ(プ)), 鋸(goe ゲェオー), 唾(toe トェオー), 嚕(lu ルー), 嘰(gi ギー), 眯(mi ミー), 柮(deot デェオッ(ト))。ご不便をおかけしますが、修正まで今しばらくお待ちください。</p>
